...
(function() {
    ...
    function ManyToOneCtrl($scope, $element, DataSource, ViewService) {
        ...
        $scope.createNestedEditor = function() {
            ...
            if ($element.parent().is("div.form-item")) {
                cell = $("<div></div>").addClass("col-md-12 form-item").append(embedded);
                row = $("<div></div>").addClass("row").append(cell);
                row.insertAfter($element.parents(".row:first"));
                return embedded;
            }
    ...
    ui.formInput('SuggestBox', 'ManyToOne', {
        ...
        template_editable: '<span class="picker-input picker-icons-2">' +
            '<input type="text" autocomplete="off" ui-can-suggest>' +
